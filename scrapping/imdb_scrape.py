...
while i <= 100:
    for film in driver.find_elements_by_class_name("mode-advanced"):
        ...
        data.append({"judul" : film.find_element_by_tag_name("h3").text.split(". ", 1)[1].split(" (", 1)[0],"genre": film.find_element_by_class_name("genre").text, "tahun" : film.find_element_by_class_name("lister-item-year").text[1:-1], "rating": film.find_element_by_tag_name("strong").text,"gambar": film.find_element_by_tag_name("img").get_attribute("src"),"waktu_scrape": datetime.now().strftime("%d/%m/%Y %H:%M:%S"), "durasi" : film.find_element_by_class_name("runtime").text})
